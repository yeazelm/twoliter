...
impl ManifestInfo {
    ...
    /// Returns the variant platform override from `[package.metadata.build-variant]`.
    pub fn variant_platform(&self) -> Option<String> {
        self.build_variant().and_then(|bv| bv.platform.clone())
    }

    /// Returns the variant runtime override from `[package.metadata.build-variant]`.
    pub fn variant_runtime(&self) -> Option<String> {
        self.build_variant().and_then(|bv| bv.runtime.clone())
    }

    /// Returns the variant family override from `[package.metadata.build-variant]`.
    pub fn variant_family(&self) -> Option<String> {
        self.build_variant().and_then(|bv| bv.family.clone())
    }

    /// Returns the variant flavor override from `[package.metadata.build-variant]`.
    pub fn variant_flavor(&self) -> Option<String> {
        self.build_variant().and_then(|bv| bv.flavor.clone())
    }
}
